...
class BasePostService(ABC):
    @abstractmethod
    def get_post_list(self, filters: PostFilters, pagination: PaginationIn) -> Iterable[Post]:
        ...
...
class ORMPostService(BasePostService):
    ...
    def get_post_list(self, filters: PostFilters, pagination: PaginationIn) -> Iterable[Post]:
        ...
        qs = PostModel.objects.filter(query)[pagination.offset: pagination.offset + pagination.limit]
